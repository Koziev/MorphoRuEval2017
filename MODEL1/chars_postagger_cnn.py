...
if run_mode == 'learn':
    ...
    if BIDIR:
        chars_per_word = max_word_len*2
        input_size = WINDOW*max_word_len*2
    else:
        chars_per_word = max_word_len
    ...
    X_data = np.zeros((n_patterns, input_size ), dtype=np.int16)
                ...
                for j in range(WINDOW):
                    ...
                    if BIDIR:
                        for ichar, c in enumerate(word_chars(word, False)):
                            char_index = j*chars_per_word + ichar
                            X_data[idata, char_index] = char2index[c]

                        for ichar, c in enumerate(word_chars(word, True)):
                            char_index = j*chars_per_word + max_word_len + ichar
                            X_data[idata, char_index] = char2index[c]
    ...
    max_nb_chars = len(all_chars)
    embedding_dims = 32
    hidden_dims = 128
    nb_filter = 128
    filter_length = 10  # было 3
    BATCH_SIZE = 64

    model = Sequential()
    model.add( Embedding(max_nb_chars, embedding_dims, input_length=input_size) )
    ...
    model_checkpoint = ModelCheckpoint(os.path.join(model_folder, 'm1_chars_postagger_net.model'), monitor='val_acc',
                                       verbose=1, save_best_only=True, mode='auto')
    early_stopping = EarlyStopping(monitor='val_acc', patience=5, verbose=1, mode='auto')

    history = model.fit(X_data, y_data, validation_split=0.1, batch_size=BATCH_SIZE, nb_epoch=100,
...
# --------------------------------------------------------------------------------------------
    ...
    winspan = int((WINDOW - 1) / 2)
    input_size = WINDOW * bits_per_word
    ...
    for line0 in rdr_corpus:
        ...
        if len(line) == 0:
            if good and len(sent) > 0:
                ...
                X_data = np.zeros((n_patterns, input_size), dtype=np.bool)
                idata = 0
                nword = len(sent)
                for iword in range(nword):
                    for j in range(WINDOW):
                        ...
                        if BIDIR:
                            for ichar, c in enumerate(limit_to(word_chars(word, False), max_word_len)):
                                if c in char2index:
                                    bit_index = j*bits_per_word + ichar * bits_per_char + char2index[c]
                                X_data[idata, bit_index] = True

                                for ichar, c in enumerate(limit_to(word_chars(word, True), max_word_len)):
                                    if c in char2index:
                                        bit_index = j*bits_per_word + max_word_len * bits_per_char + ichar * bits_per_char + char2index[c]
                                    X_data[idata, bit_index] = True
                        else:
